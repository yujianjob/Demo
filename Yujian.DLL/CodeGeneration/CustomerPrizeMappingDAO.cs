...
namespace YuJian.WeiXin.DataAccess
{
    ...
    public partial class CustomerPrizeMappingDAO : BaseDAO<BasicUserInfo>, ICRUDable<CustomerPrizeMappingEntity>, IQueryable<CustomerPrizeMappingEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public CustomerPrizeMappingDAO(BasicUserInfo pUserInfo)
        ...
        public void Create(CustomerPrizeMappingEntity pEntity, IDbTransaction pTran)
        {
            ...
            strSql.Append("insert into [CustomerPrizeMapping](");
            strSql.Append("[Openid],[PrizeID],[Enable],[CreateBy],[CreateTime],[LastUpdateBy],[LastUpdateTime],[IsDelete])");
			...
			parameters[2].Value = pEntity.Enable;
        ...
        public CustomerPrizeMappingEntity GetByID(object pID)
        {
            ...
            sql.AppendFormat("select * from [CustomerPrizeMapping] where ID='{0}' and IsDelete=0 ", id.ToString());
            //读取数据
            CustomerPrizeMappingEntity m = null;
        ...
        public CustomerPrizeMappingEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [CustomerPrizeMapping] where isdelete=0");
            //读取数据
            List<CustomerPrizeMappingEntity> list = new List<CustomerPrizeMappingEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
                    CustomerPrizeMappingEntity m;
                    this.Load(rdr, out m);
                    list.Add(m);
                }
            }
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [CustomerPrizeMapping] set LastUpdateTime=@LastUpdateTime,LastUpdateBy=@LastUpdateBy,IsDelete=1 where ID=@ID;");
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [CustomerPrizeMapping] set LastUpdateTime='"+DateTime.Now.ToString()+"',LastUpdateBy="+CurrentUserInfo.UserID+",IsDelete=1 where ID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        public PagedQueryResult<CustomerPrizeMappingEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            ...
            pagedSql.AppendFormat(") as ___rn,* from [CustomerPrizeMapping] where isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [CustomerPrizeMapping] where isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<CustomerPrizeMappingEntity> result = new PagedQueryResult<CustomerPrizeMappingEntity>();
            List<CustomerPrizeMappingEntity> list = new List<CustomerPrizeMappingEntity>();
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(CustomerPrizeMappingEntity pQueryEntity)
        { 
            //获取非空属性数量
            ...
            if (pQueryEntity.Enable!=null)
                lstWhereCondition.Add(new EqualsCondition() { FieldName = "Enable", Value = pQueryEntity.Enable });
        ...
        protected void Load(SqlDataReader pReader, out CustomerPrizeMappingEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new CustomerPrizeMappingEntity();
			...
			{
				pInstance.PrizeID =   Convert.ToInt32(pReader["PrizeID"]);
			}
			if (pReader["Enable"] != DBNull.Value)
			{
				pInstance.Enable =   Convert.ToInt32(pReader["Enable"]);
			}

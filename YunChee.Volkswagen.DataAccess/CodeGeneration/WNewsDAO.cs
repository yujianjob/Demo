...
namespace Yunchee.Volkswagen.DataAccess
{
    ...
    public partial class WNewsDAO : BaseDAO<BasicUserInfo>, ICRUDable<WNewsEntity>, IQueryable<WNewsEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public WNewsDAO(BasicUserInfo pUserInfo)
        ...
        public void Create(WNewsEntity pEntity)
        {
        ...
        public void Create(WNewsEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
            ...
            strSql.Append("insert into [WNews](");
            strSql.Append("[Title],[Description],[PictureUrl],[OriginalUrl],[Content],[SortIndex],[EventID],[InformationID],[ClientID],[CreateBy],[CreateTime],[LastUpdateBy],[LastUpdateTime],[IsDelete])");
            ...
            SqlParameter[] parameters = 
            {
					...
					new SqlParameter("@InformationID",SqlDbType.Int),
			...
			parameters[2].Value = pEntity.PictureUrl;
			parameters[3].Value = pEntity.OriginalUrl;
			parameters[4].Value = pEntity.Content;
			parameters[5].Value = pEntity.SortIndex;
			parameters[6].Value = pEntity.EventID;
			parameters[7].Value = pEntity.InformationID;
        ...
        public WNewsEntity GetByID(object pID)
        {
            //参数检查
            if (pID == null)
                return null;
            ...
            sql.AppendFormat("select * from [WNews] where ID='{0}' and IsDelete=0 ", id.ToString());
            //读取数据
            WNewsEntity m = null;
        ...
        /// </summary>
        /// <returns></returns>
        public WNewsEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [WNews] where isdelete=0");
            //读取数据
            List<WNewsEntity> list = new List<WNewsEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(WNewsEntity pEntity , IDbTransaction pTran)
        {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(WNewsEntity pEntity , IDbTransaction pTran,bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
            ...
            //组织参数化SQL
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [WNews] set ");
            ...
            if (pIsUpdateNullField || pEntity.InformationID!=null)
        ...
        /// </summary>
        /// <param name="pEntity">实体实例</param>
        public void Update(WNewsEntity pEntity )
        {
        ...
        public void Delete(WNewsEntity pEntity)
        {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(WNewsEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [WNews] set LastUpdateTime=@LastUpdateTime,LastUpdateBy=@LastUpdateBy,IsDelete=1 where ID=@ID;");
        ...
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(WNewsEntity[] pEntities, IDbTransaction pTran)
        {
            //整理主键值
        ...
        /// </summary>
        /// <param name="pEntities">实体实例数组</param>
        public void Delete(WNewsEntity[] pEntities)
        { 
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [WNews] set LastUpdateTime='"+DateTime.Now.ToString()+"',LastUpdateBy="+CurrentUserInfo.UserID+",IsDelete=1 where ID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        /// <returns></returns>
        public WNewsEntity[] Query(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys)
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [WNews] where isdelete=0 ");
        ...
        /// <returns></returns>
        public PagedQueryResult<WNewsEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            //组织SQL
            ...
            pagedSql.AppendFormat(") as ___rn,* from [WNews] where isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [WNews] where isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<WNewsEntity> result = new PagedQueryResult<WNewsEntity>();
            List<WNewsEntity> list = new List<WNewsEntity>();
        ...
        public WNewsEntity[] QueryByEntity(WNewsEntity pQueryEntity, OrderBy[] pOrderBys)
        {
        ...
        /// <param name="pOrderBys">排序组合</param>
        /// <returns>符合条件的实体集</returns>
        public PagedQueryResult<WNewsEntity> PagedQueryByEntity(WNewsEntity pQueryEntity, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            IWhereCondition[] queryWhereCondition = GetWhereConditionByEntity( pQueryEntity);
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(WNewsEntity pQueryEntity)
        { 
            //获取非空属性数量
            ...
            if (pQueryEntity.InformationID!=null)
                lstWhereCondition.Add(new EqualsCondition() { FieldName = "InformationID", Value = pQueryEntity.InformationID });
        ...
        protected void Load(SqlDataReader pReader, out WNewsEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new WNewsEntity();
			...
			if (pReader["InformationID"] != DBNull.Value)
			{
				pInstance.InformationID =   Convert.ToInt32(pReader["InformationID"]);
			}

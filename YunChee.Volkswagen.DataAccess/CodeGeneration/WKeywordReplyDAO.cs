...
namespace Yunchee.Volkswagen.DataAccess
{
    ...
    public partial class WKeywordReplyDAO : BaseDAO<BasicUserInfo>, ICRUDable<WKeywordReplyEntity>, IQueryable<WKeywordReplyEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public WKeywordReplyDAO(BasicUserInfo pUserInfo)
        ...
        public void Create(WKeywordReplyEntity pEntity, IDbTransaction pTran)
        {
            ...
            strSql.Append("[ParentID],[Keyword],[Description],[ReplyTypeID],[Text],[Tag],[Enabled],[Level],[SortIndex],[ApplicationID],[CreateBy],[CreateTime],[LastUpdateBy],[LastUpdateTime],[IsDelete])");
			...
			parameters[1].Value = pEntity.Keyword;
			parameters[2].Value = pEntity.Description;
			parameters[3].Value = pEntity.ReplyTypeID;
			parameters[4].Value = pEntity.Text;
			parameters[5].Value = pEntity.Tag;
			parameters[6].Value = pEntity.Enabled;
			parameters[7].Value = pEntity.Level;
			parameters[8].Value = pEntity.SortIndex;
			parameters[9].Value = pEntity.ApplicationID;
        ...
        public WKeywordReplyEntity GetByID(object pID)
        {
            ...
            sql.AppendFormat("select * from [WKeywordReply] where ID='{0}' and IsDelete=0 ", id.ToString());
            //读取数据
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(WKeywordReplyEntity pEntity , IDbTransaction pTran,bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [WKeywordReply] set LastUpdateTime=@LastUpdateTime,LastUpdateBy=@LastUpdateBy,IsDelete=1 where ID=@ID;");
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [WKeywordReply] set LastUpdateTime='"+DateTime.Now.ToString()+"',LastUpdateBy="+CurrentUserInfo.UserID+",IsDelete=1 where ID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)

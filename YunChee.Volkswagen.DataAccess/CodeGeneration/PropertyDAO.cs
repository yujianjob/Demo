...
namespace Yunchee.Volkswagen.DataAccess
{
    ...
    public partial class PropertyDAO : BaseDAO<BasicUserInfo>, ICRUDable<PropertyEntity>, IQueryable<PropertyEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public PropertyDAO(BasicUserInfo pUserInfo)
        ...
        public void Create(PropertyEntity pEntity)
        {
        ...
        public void Create(PropertyEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
            ...
            strSql.Append("[Code],[Name],[Type],[Scope],[IsRequired],[IsUnique],[DefaultValue],[MaxLength],[Tips],[ClientID],[CreateBy],[CreateTime],[LastUpdateBy],[LastUpdateTime],[IsDelete],[ID])");
            strSql.Append(" values (");
            strSql.Append("@Code,@Name,@Type,@Scope,@IsRequired,@IsUnique,@DefaultValue,@MaxLength,@Tips,@ClientID,@CreateBy,@CreateTime,@LastUpdateBy,@LastUpdateTime,@IsDelete,@ID)");            

			int? pkInt = pEntity.ID;
			...
			parameters[4].Value = pEntity.IsRequired;
			parameters[5].Value = pEntity.IsUnique;
			parameters[6].Value = pEntity.DefaultValue;
			parameters[7].Value = pEntity.MaxLength;
			parameters[8].Value = pEntity.Tips;
        ...
        public PropertyEntity GetByID(object pID)
        {
            //参数检查
            if (pID == null)
                return null;
            ...
            sql.AppendFormat("select * from [Property] where ID='{0}' and IsDelete=0 ", id.ToString());
            //读取数据
            PropertyEntity m = null;
        ...
        /// </summary>
        /// <returns></returns>
        public PropertyEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [Property] where isdelete=0");
            //读取数据
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(PropertyEntity pEntity , IDbTransaction pTran)
        {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(PropertyEntity pEntity , IDbTransaction pTran,bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
            ...
            if (pIsUpdateNullField || pEntity.IsUnique!=null)
                strSql.Append( "[IsUnique]=@IsUnique,");
            ...
            if (pIsUpdateNullField || pEntity.Tips!=null)
        ...
        /// </summary>
        /// <param name="pEntity">实体实例</param>
        public void Update(PropertyEntity pEntity )
        {
        ...
        public void Delete(PropertyEntity pEntity)
        {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(PropertyEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [Property] set LastUpdateTime=@LastUpdateTime,LastUpdateBy=@LastUpdateBy,IsDelete=1 where ID=@ID;");
            ...
            { 
                ...
                new SqlParameter{ParameterName="@ID",SqlDbType=SqlDbType.Int,Value=pID}
            };
            //执行语句
            int result = 0;
            if (pTran != null)
        ...
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(PropertyEntity[] pEntities, IDbTransaction pTran)
        {
            //整理主键值
        ...
        /// </summary>
        /// <param name="pEntities">实体实例数组</param>
        public void Delete(PropertyEntity[] pEntities)
        { 
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [Property] set LastUpdateTime='"+DateTime.Now.ToString()+"',LastUpdateBy="+CurrentUserInfo.UserID+",IsDelete=1 where ID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        /// <returns></returns>
        public PropertyEntity[] Query(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys)
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [Property] where isdelete=0 ");
        ...
        /// <returns></returns>
        public PagedQueryResult<PropertyEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            //组织SQL
            ...
            pagedSql.AppendFormat(") as ___rn,* from [Property] where isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [Property] where isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<PropertyEntity> result = new PagedQueryResult<PropertyEntity>();
            List<PropertyEntity> list = new List<PropertyEntity>();
        ...
        public PropertyEntity[] QueryByEntity(PropertyEntity pQueryEntity, OrderBy[] pOrderBys)
        {
        ...
        /// <param name="pOrderBys">排序组合</param>
        /// <returns>符合条件的实体集</returns>
        public PagedQueryResult<PropertyEntity> PagedQueryByEntity(PropertyEntity pQueryEntity, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            IWhereCondition[] queryWhereCondition = GetWhereConditionByEntity( pQueryEntity);
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(PropertyEntity pQueryEntity)
        { 
            //获取非空属性数量
            ...
            if (pQueryEntity.IsUnique!=null)
                lstWhereCondition.Add(new EqualsCondition() { FieldName = "IsUnique", Value = pQueryEntity.IsUnique });
            ...
            if (pQueryEntity.Tips!=null)
                lstWhereCondition.Add(new EqualsCondition() { FieldName = "Tips", Value = pQueryEntity.Tips });
        ...
        protected void Load(SqlDataReader pReader, out PropertyEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new PropertyEntity();
			...
			if (pReader["IsUnique"] != DBNull.Value)
			{
				pInstance.IsUnique =   Convert.ToInt32(pReader["IsUnique"]);
			}
			...
			if (pReader["Tips"] != DBNull.Value)
			{
				pInstance.Tips =  Convert.ToString(pReader["Tips"]);
			}

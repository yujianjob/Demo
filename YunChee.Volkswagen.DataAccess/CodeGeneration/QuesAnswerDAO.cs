...
namespace Yunchee.Volkswagen.DataAccess
{
    ...
    public partial class QuesAnswerDAO : BaseDAO<BasicUserInfo>, ICRUDable<QuesAnswerEntity>, IQueryable<QuesAnswerEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public QuesAnswerDAO(BasicUserInfo pUserInfo)
        ...
        public void Create(QuesAnswerEntity pEntity, IDbTransaction pTran)
        {
            ...
            strSql.Append("insert into [QuesAnswer](");
            strSql.Append("[QuestionID],[OptionID],[OptionValue],[CustomerID],[CreateBy],[CreateTime],[LastUpdateBy],[LastUpdateTime],[IsDelete])");
			...
			parameters[1].Value = pEntity.OptionID;
        ...
        public QuesAnswerEntity GetByID(object pID)
        {
            //参数检查
            if (pID == null)
                return null;
            ...
            sql.AppendFormat("select * from [QuesAnswer] where ID='{0}' and IsDelete=0 ", id.ToString());
            //读取数据
            QuesAnswerEntity m = null;
        ...
        /// </summary>
        /// <returns></returns>
        public QuesAnswerEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [QuesAnswer] where isdelete=0");
            //读取数据
            List<QuesAnswerEntity> list = new List<QuesAnswerEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(QuesAnswerEntity pEntity , IDbTransaction pTran,bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
            ...
            //组织参数化SQL
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [QuesAnswer] set ");
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(QuesAnswerEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [QuesAnswer] set LastUpdateTime=@LastUpdateTime,LastUpdateBy=@LastUpdateBy,IsDelete=1 where ID=@ID;");
        ...
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(QuesAnswerEntity[] pEntities, IDbTransaction pTran)
        {
            //整理主键值
        ...
        /// </summary>
        /// <param name="pEntities">实体实例数组</param>
        public void Delete(QuesAnswerEntity[] pEntities)
        { 
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [QuesAnswer] set LastUpdateTime='"+DateTime.Now.ToString()+"',LastUpdateBy="+CurrentUserInfo.UserID+",IsDelete=1 where ID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        /// <returns></returns>
        public QuesAnswerEntity[] Query(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys)
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [QuesAnswer] where isdelete=0 ");
        ...
        /// <returns></returns>
        public PagedQueryResult<QuesAnswerEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            //组织SQL
            ...
            pagedSql.AppendFormat(") as ___rn,* from [QuesAnswer] where isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [QuesAnswer] where isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<QuesAnswerEntity> result = new PagedQueryResult<QuesAnswerEntity>();
            List<QuesAnswerEntity> list = new List<QuesAnswerEntity>();
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(QuesAnswerEntity pQueryEntity)
        { 
            //获取非空属性数量
        ...
        protected void Load(SqlDataReader pReader, out QuesAnswerEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new QuesAnswerEntity();
			...
			{
				pInstance.OptionID =   Convert.ToInt32(pReader["OptionID"]);
			}

...
namespace Yunchee.Volkswagen.DataAccess
{
    ...
    public partial class WAutoReplyDAO : BaseDAO<BasicUserInfo>, ICRUDable<WAutoReplyEntity>, IQueryable<WAutoReplyEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public WAutoReplyDAO(BasicUserInfo pUserInfo)
        ...
        public WAutoReplyEntity GetByID(object pID)
        {
            ...
            sql.AppendFormat("select * from [WAutoReply] where ID='{0}' and IsDelete=0 ", id.ToString());
            //读取数据
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(WAutoReplyEntity pEntity , IDbTransaction pTran,bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [WAutoReply] set LastUpdateTime=@LastUpdateTime,LastUpdateBy=@LastUpdateBy,IsDelete=1 where ID=@ID;");
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [WAutoReply] set LastUpdateTime='"+DateTime.Now.ToString()+"',LastUpdateBy="+CurrentUserInfo.UserID+",IsDelete=1 where ID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)

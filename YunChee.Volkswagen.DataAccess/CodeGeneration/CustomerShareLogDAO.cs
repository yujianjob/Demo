...
{
    /// <summary>
    /// 数据访问： 0110客户分享日志表 CustomerShareLog 
    /// 表CustomerShareLog的数据访问类     
    /// 1.实现ICRUDable接口
    /// 2.实现IQueryable接口
    /// 3.实现Load方法
    /// </summary>
    public partial class CustomerShareLogDAO : BaseDAO<BasicUserInfo>, ICRUDable<CustomerShareLogEntity>, IQueryable<CustomerShareLogEntity>
    {
        #region 构造函数
        /// <summary>
        /// 构造函数 
        /// </summary>
        public CustomerShareLogDAO(BasicUserInfo pUserInfo)
        ...
        public void Create(CustomerShareLogEntity pEntity)
        {
        ...
        public void Create(CustomerShareLogEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
            ...
            strSql.Append("insert into [CustomerShareLog](");
            strSql.Append("[CustomerID],[OperationID1],[OperationID2],[OperationID3],[Type],[ObjectID],[WxId],[WxOpenId],[CreateBy],[CreateTime],[LastUpdateBy],[LastUpdateTime],[IsDelete])");
			...
			parameters[6].Value = pEntity.WxId;
			parameters[7].Value = pEntity.WxOpenId;
        ...
        public CustomerShareLogEntity GetByID(object pID)
        {
            //参数检查
            if (pID == null)
                return null;
            ...
            sql.AppendFormat("select * from [CustomerShareLog] where ID='{0}' and IsDelete=0 ", id.ToString());
            //读取数据
            CustomerShareLogEntity m = null;
        ...
        /// </summary>
        /// <returns></returns>
        public CustomerShareLogEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [CustomerShareLog] where isdelete=0");
            //读取数据
            List<CustomerShareLogEntity> list = new List<CustomerShareLogEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
                    CustomerShareLogEntity m;
                    this.Load(rdr, out m);
                    list.Add(m);
                }
            }
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(CustomerShareLogEntity pEntity , IDbTransaction pTran)
        {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(CustomerShareLogEntity pEntity , IDbTransaction pTran,bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
            ...
            //组织参数化SQL
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [CustomerShareLog] set ");
        ...
        /// </summary>
        /// <param name="pEntity">实体实例</param>
        public void Update(CustomerShareLogEntity pEntity )
        {
        ...
        public void Delete(CustomerShareLogEntity pEntity)
        {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(CustomerShareLogEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [CustomerShareLog] set LastUpdateTime=@LastUpdateTime,LastUpdateBy=@LastUpdateBy,IsDelete=1 where ID=@ID;");
        ...
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(CustomerShareLogEntity[] pEntities, IDbTransaction pTran)
        {
            //整理主键值
        ...
        /// </summary>
        /// <param name="pEntities">实体实例数组</param>
        public void Delete(CustomerShareLogEntity[] pEntities)
        { 
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [CustomerShareLog] set LastUpdateTime='"+DateTime.Now.ToString()+"',LastUpdateBy="+CurrentUserInfo.UserID+",IsDelete=1 where ID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        /// <returns></returns>
        public CustomerShareLogEntity[] Query(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys)
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [CustomerShareLog] where isdelete=0 ");
        ...
        /// <returns></returns>
        public PagedQueryResult<CustomerShareLogEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            //组织SQL
            ...
            pagedSql.AppendFormat(") as ___rn,* from [CustomerShareLog] where isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [CustomerShareLog] where isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<CustomerShareLogEntity> result = new PagedQueryResult<CustomerShareLogEntity>();
            List<CustomerShareLogEntity> list = new List<CustomerShareLogEntity>();
        ...
        public CustomerShareLogEntity[] QueryByEntity(CustomerShareLogEntity pQueryEntity, OrderBy[] pOrderBys)
        {
        ...
        /// <param name="pOrderBys">排序组合</param>
        /// <returns>符合条件的实体集</returns>
        public PagedQueryResult<CustomerShareLogEntity> PagedQueryByEntity(CustomerShareLogEntity pQueryEntity, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            IWhereCondition[] queryWhereCondition = GetWhereConditionByEntity( pQueryEntity);
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(CustomerShareLogEntity pQueryEntity)
        { 
            //获取非空属性数量
        ...
        protected void Load(SqlDataReader pReader, out CustomerShareLogEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new CustomerShareLogEntity();

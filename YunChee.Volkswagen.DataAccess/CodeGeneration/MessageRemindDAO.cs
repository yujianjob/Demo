...
namespace Yunchee.Volkswagen.DataAccess
{
    ...
    public partial class MessageRemindDAO : BaseDAO<BasicUserInfo>, ICRUDable<MessageRemindEntity>, IQueryable<MessageRemindEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public MessageRemindDAO(BasicUserInfo pUserInfo)
        ...
        public void Create(MessageRemindEntity pEntity)
        {
        ...
        public void Create(MessageRemindEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
            ...
            strSql.Append("[MessageType],[Status],[OrderID],[ClientID],[CreateBy],[CreateTime],[LastUpdateBy],[LastUpdateTime],[IsDelete])");
        ...
        public MessageRemindEntity GetByID(object pID)
        {
            //参数检查
            if (pID == null)
                return null;
            ...
            sql.AppendFormat("select * from [MessageRemind] where ID='{0}' and IsDelete=0 ", id.ToString());
            //读取数据
            MessageRemindEntity m = null;
        ...
        /// </summary>
        /// <returns></returns>
        public MessageRemindEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [MessageRemind] where isdelete=0");
            //读取数据
            List<MessageRemindEntity> list = new List<MessageRemindEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
                    MessageRemindEntity m;
                    this.Load(rdr, out m);
                    list.Add(m);
                }
            }
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(MessageRemindEntity pEntity, IDbTransaction pTran)
        {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(MessageRemindEntity pEntity, IDbTransaction pTran, bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        /// </summary>
        /// <param name="pEntity">实体实例</param>
        public void Update(MessageRemindEntity pEntity)
        {
        ...
        public void Delete(MessageRemindEntity pEntity)
        {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(MessageRemindEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [MessageRemind] set LastUpdateTime=@LastUpdateTime,LastUpdateBy=@LastUpdateBy,IsDelete=1 where ID=@ID;");
        ...
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(MessageRemindEntity[] pEntities, IDbTransaction pTran)
        {
            //整理主键值
        ...
        /// </summary>
        /// <param name="pEntities">实体实例数组</param>
        public void Delete(MessageRemindEntity[] pEntities)
        {
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran)
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [MessageRemind] set LastUpdateTime='" + DateTime.Now.ToString() + "',LastUpdateBy=" + CurrentUserInfo.UserID + ",IsDelete=1 where ID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;
            if (pTran == null)
        ...
        /// <returns></returns>
        public MessageRemindEntity[] Query(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys)
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [MessageRemind] where isdelete=0 ");
        ...
        /// <returns></returns>
        public PagedQueryResult<MessageRemindEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            //组织SQL
            ...
            pagedSql.AppendFormat(") as ___rn,* from [MessageRemind] where isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [MessageRemind] where isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<MessageRemindEntity> result = new PagedQueryResult<MessageRemindEntity>();
            List<MessageRemindEntity> list = new List<MessageRemindEntity>();
        ...
        public MessageRemindEntity[] QueryByEntity(MessageRemindEntity pQueryEntity, OrderBy[] pOrderBys)
        {
        ...
        /// <param name="pOrderBys">排序组合</param>
        /// <returns>符合条件的实体集</returns>
        public PagedQueryResult<MessageRemindEntity> PagedQueryByEntity(MessageRemindEntity pQueryEntity, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            IWhereCondition[] queryWhereCondition = GetWhereConditionByEntity(pQueryEntity);
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(MessageRemindEntity pQueryEntity)
        {
            //获取非空属性数量
        ...
        protected void Load(SqlDataReader pReader, out MessageRemindEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new MessageRemindEntity();
